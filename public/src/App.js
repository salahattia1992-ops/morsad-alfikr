...
const THINK_TANKS = [
  ...
  { id: 15, nameAr: "المجلس الأوروبي للعلاقات الخارجية", region: "أوروبا", flag: "🇪🇺", rss: "https://ecfr.eu/feed/", color: "#003399", accent: "#ffcc00" },
  { id: 16, nameAr: "معهد بروغل", region: "أوروبا", flag: "🇧🇪", rss: "https://www.bruegel.org/rss", color: "#1a1a6e", accent: "#4fc3f7" },
  { id: 17, nameAr: "معهد ستوكهولم لأبحاث السلام", region: "أوروبا", flag: "🇸🇪", rss: "https://www.sipri.org/rss.xml", color: "#006aa7", accent: "#fecc02" },
  { id: 18, nameAr: "صندوق مارشال الألماني", region: "أوروبا", flag: "🇩🇪", rss: "https://www.gmfus.org/feed", color: "#1a1a1a", accent: "#dd0000" },
  { id: 19, nameAr: "معهد مونتين", region: "أوروبا", flag: "🇫🇷", rss: "https://www.institutmontaigne.org/en/rss.xml", color: "#002395", accent: "#ed2939" },
  { id: 20, nameAr: "المعهد الإيطالي للدراسات السياسية", region: "أوروبا", flag: "🇮🇹", rss: "https://www.ispionline.it/en/rss", color: "#009246", accent: "#ce2b37" },
  { id: 21, nameAr: "نادي فالداي للنقاش", region: "روسيا", flag: "🇷🇺", rss: "https://valdaiclub.com/rss/", color: "#cc0000", accent: "#ffd700" },
  { id: 22, nameAr: "المجلس الروسي للشؤون الدولية", region: "روسيا", flag: "🇷🇺", rss: "https://russiancouncil.ru/en/rss/", color: "#1a237e", accent: "#ef5350" },
  { id: 23, nameAr: "معاهد شنغهاي للدراسات الدولية", region: "آسيا", flag: "🇨🇳", rss: "https://www.siis.org.cn/rss", color: "#c0392b", accent: "#f39c12" },
  { id: 24, nameAr: "معهد جنوب شرق آسيا", region: "آسيا", flag: "🇸🇬", rss: "https://www.iseas.edu.sg/medias/feed/", color: "#003d6b", accent: "#e74c3c" },
  { id: 25, nameAr: "مؤسسة أبحاث أوبزرفر", region: "آسيا", flag: "🇮🇳", rss: "https://www.orfonline.org/feed/", color: "#ff6600", accent: "#138808" },
  { id: 26, nameAr: "مركز الجزيرة للدراسات", region: "عربي", flag: "🇶🇦", rss: "https://studies.aljazeera.net/ar/rss.xml", color: "#4b0082", accent: "#ffd700" },
  { id: 27, nameAr: "مركز بروكينغز الدوحة", region: "عربي", flag: "🇶🇦", rss: "https://www.brookings.edu/center/brookings-doha-center/feed/", color: "#005288", accent: "#c8a951" },
  { id: 28, nameAr: "معهد واشنطن لسياسة الشرق الأدنى", region: "عربي", flag: "🇺🇸", rss: "https://www.washingtoninstitute.org/rss/policy-analysis", color: "#1a3a6b", accent: "#4d8bf5" },
  { id: 29, nameAr: "مركز كارنيغي الشرق الأوسط", region: "عربي", flag: "🇱🇧", rss: "https://carnegie-mec.org/rss/", color: "#c41230", accent: "#ffd700" },
  { id: 30, nameAr: "منتدى الفكر العربي", region: "عربي", flag: "🇯🇴", rss: "https://www.atf.org.jo/feed", color: "#007a3d", accent: "#ce1126" },
];

const REGIONS = ["الكل", "أمريكا", "أوروبا", "روسيا", "آسيا", "عربي"];
const AUTO_REFRESH_MINUTES = 30;

async function fetchRSS(tankRss) {
  const proxyUrl = `/api/rss?url=${encodeURIComponent(tankRss)}`;
  try {
...
export default function App() {
  ...
  const toggleFavorite = (articleId) => {
    setFavorites(prev => {
      ...
      showNotif(prev.includes(articleId) ? "إزالة من المفضلة" : "✨ تمت الإضافة للمفضلة");
      return updated;
    });
  };
  ...
  const doFetch = useCallback(async (tanks, silent = false) => {
    ...
    setLoading(false);
    setRefreshing(false);
    if (allArticles.length > 0) showNotif(`✅ تم تحميل ${allArticles.length} مقالة`);
    else showNotif("⚠️ لم تُحمَّل مقالات، تحقق من الاتصال", "error");
  }, []);
  ...
  const ArticleCard = ({ article }) => {
    ...
    return (
      <article className="card" style={{ borderRadius: 18, background: isExpanded ? `linear-gradient(135deg, ${article.tankColor}22, rgba(8,8,20,0.98))` : "rgba(255,255,255,0.03)", overflow: "hidden", border: "1px solid rgba(255,255,255,0.06)" }}>
    ...
    <div dir="rtl" style={{ minHeight: "100vh", background: "linear-gradient(135deg, #030308 0%, #0a0a18 50%, #050510 100%)", fontFamily: "'Cairo', sans-serif", color: "#e8e8f0" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .card{transition:all 0.3s ease}.card:hover{transform:translateY(-4px);border-color:rgba(212,175,55,0.25)!important;box-shadow:0 20px 60px rgba(0,0,0,0.5)}
      ...
      <header style={{ background: "rgba(5,5,18,0.95)", borderBottom: "1px solid rgba(212,175,55,0.15)", backdropFilter: "blur(20px)", position: "sticky", top: 0, zIndex: 100, padding: "0 20px" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "14px 0", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            {lastRefresh && <div style={{ fontSize: 10, color: "#444" }}>آخر تحديث: {lastRefresh.toLocaleTimeString("ar-SA")}</div>}
            <div style={{ position: "relative" }}>
              <input placeholder="ابحث..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ padding: "8px 32px 8px 12px", borderRadius: 10, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#e8e8f0", fontSize: 12, outline: "none", width: 170 }} />
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "24px 20px" }}>
        {activeTab === "articles" && (
          <>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              {REGIONS.map(r => (
                <button key={r} onClick={() => { setSelectedRegion(r); setSelectedTank(null); }} style={{ padding: "7px 16px", borderRadius: 30, fontSize: 12, background: selectedRegion === r ? "linear-gradient(135deg, #d4af37, #8b6914)" : "rgba(255,255,255,0.05)", color: selectedRegion === r ? "#000" : "#aaa", fontWeight: selectedRegion === r ? 700 : 400, border: "none", cursor: "pointer" }}>{r === "الكل" ? "🌍 " : ""}{r}</button>
              ))}
            </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {filteredTanks.map(tank => (
                  <div key={tank.id} onClick={() => setSelectedTank(selectedTank === tank.id ? null : tank.id)} style={{ padding: "6px 12px", borderRadius: 10, background: selectedTank === tank.id ? `linear-gradient(135deg, ${tank.color}bb, ${tank.color}77)` : "rgba(255,255,255,0.03)", border: `1px solid ${selectedTank === tank.id ? "#d4af37" : "rgba(255,255,255,0.08)"}`, display: "flex", alignItems: "center", gap: 7, cursor: "pointer", transition: "all 0.2s" }}>
                    ...
                    <span style={{ fontSize: 11, color: selectedTank === tank.id ? "#fff" : "#aaa" }}>{tank.nameAr}</span>
                    ...
                    <span className={fetchStatus[tank.id] === "loading" ? "pulse" : ""} style={{ width: 5, height: 5, borderRadius: "50%", flexShrink: 0, background: fetchStatus[tank.id] === "ok" ? "#3ddc97" : fetchStatus[tank.id] === "loading" ? "#d4af37" : fetchStatus[tank.id] === "error" ? "#ff4d6d" : "#333" }} />
                  </div>
                ))}
              </div>
            </div>
            {loading && <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 18 }}>{Array(6).fill(0).map((_, i) => <div key={i} className="shimmer" style={{ borderRadius: 16, height: 250 }} />)}</div>}
            {!loading && filteredArticles.length === 0 && (
              <div style={{ textAlign: "center", padding: "80px 20px" }}>
                <div style={{ fontSize: 56, marginBottom: 16 }}>🔭</div>
                <h3 style={{ color: "#d4af37", fontSize: 18, marginBottom: 8 }}>لا توجد مقالات</h3>
                <p style={{ color: "#555", fontSize: 13, marginBottom: 20 }}>جارٍ الاتصال بالمراكز البحثية...</p>
            ...
            {!loading && filteredArticles.length > 0 && (
              <>
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 18 }}>
                  <span style={{ fontSize: 12, color: "#555" }}><span style={{ color: "#d4af37", fontWeight: 700 }}>{filteredArticles.length}</span> مقالة حية</span>
        ...
        {activeTab === "favorites" && (
          <>
            ...
            <p style={{ fontSize: 13, color: "#555", marginBottom: 24 }}>{favoriteArticles.length} مقالة</p>
            {favoriteArticles.length === 0 ? <div style={{ textAlign: "center", padding: "80px 20px" }}><div style={{ fontSize: 56 }}>☆</div><h3 style={{ color: "#888", fontSize: 18, marginTop: 16 }}>لا توجد مقالات محفوظة</h3></div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 18 }}>{favoriteArticles.map(a => <ArticleCard key={a.id} article={a} />)}</div>}
          </>
        )}
        {activeTab === "stats" && (
          <>
            <h2 style={{ fontSize: 18, color: "#d4af37", marginBottom: 20 }}>📊 إحصاءات المراكز البحثية</h2>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 16, border: "1px solid rgba(255,255,255,0.05)", padding: 20 }}>
              <div style={{ fontSize: 13, color: "#d4af37", marginBottom: 16, fontWeight: 700 }}>نشاط المراكز</div>
              {statsData.length === 0 ? <p style={{ color: "#555", fontSize: 13 }}>لا توجد بيانات بعد</p> : statsData.map(tank => {
                const pct = (tank.count / Math.max(...statsData.map(t => t.count), 1)) * 100;
                return (
                  <div key={tank.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    <span style={{ fontSize: 14, flexShrink: 0 }}>{tank.flag}</span>
                    <div style={{ width: 140, fontSize: 11, color: "#aaa", flexShrink: 0, textAlign: "right" }}>{tank.nameAr}</div>
